<?php 
 ...
 $slt = mysqli_query($conn, "SELECT * FROM usr WHERE unic_id='{$sessid}'");
  ...
  $target = "".$rows['fname'].'_'.$rows['lname'].$pass."profilepic".basename($_FILES['change_pp']['name']);
   ...
   else if(move_uploaded_file($_FILES['change_pp']['tmp_name'], $target)){ 
